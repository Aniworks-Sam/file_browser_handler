const folderSearch = (data1:any, folderChainTemp:any, currentFolder:any) => {
  ...
  for (let i = 0; i < data1.length; i++) {
    ...
    if (folder.id === currentFolder) {
      ...
      let returnValues:any = folderSearch(
        folder.files,
        folderChainTemp,
        currentFolder
      );
...
export const findFile = (data:any, fileId:any) => {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (folder.id === fileId) {
      ...
      let returnValues:any = findFile(folder.files, fileId);
      if (returnValues) {
        return returnValues;
      }
    }
